export default class Gemini {
  ...
  static init(matrix) {
    ...
    Gemini.#prompt = `Instrucciones: Recibe una matriz de "Naval Battle" donde cada celda tiene un valor: 1 = disparo acertado (parte del barco), -1 = disparo fallido (bomba, sin barco) y 0 = celda intacta (sin disparo). Analiza la matriz y elige la posición [row, col] óptima para disparar. 
     Responde SIEMPRE con el siguiente formato EXACTO: {"message":"Mensaje de ánimo y juego", "position":[row, col]} sin saltos de línea, sin formato extra, solo ese string puro. Esta es la matriz: ${matrix}`;
